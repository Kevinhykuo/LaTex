...
%	\subsection{Cartesian and Polar Forms of the Cauchy-Riemann Equations} \label{Cauchy-Riemann Equations (Cartesian and Polar) Subsection - Complex}
	...
	\begin{example}
		...
		\begin{proof}{\color{Grey}
			...
			Note:
			\begin{align*}
				f'(z_0) = 0 \implies u_x + iv_x = 0 \implies v_y - iu_y = 0 \implies u_x = u_y = v_x = v_y = 0
			\end{align*}
			Hence, we can see that \(f'(z_0)\) is required for \(u(x,y)\) and \(v(x,y)\) to exist and be orthogonal.
		}
		\end{proof}
	\end{example}
