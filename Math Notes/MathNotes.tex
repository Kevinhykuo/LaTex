...
%	\subsection{Cartesian and Polar Forms of the Cauchy-Riemann Equations} \label{Cauchy-Riemann Equations (Cartesian and Polar) Subsection - Complex}
	...
	\begin{question}
		...
		\begin{align*}
			...
			\iff f'(z_0) \text{ exists }
		\end{align*}
		Should the Cartesian one also be $\iff$ instead of $\implies$? 
		Yes, but only under the condition \(z_0 \neq 0\) as we will see in \cref{f(z) satisfy Cauchy-Riemann but f'(z_0) does not exist Example - Complex}.
	\end{question}
	...
	\label{f(z) satisfy Cauchy-Riemann but f'(z_0) does not exist Example - Complex}
		...
		{\color{Grey}
			...
			\end{align*}
			As we can see, the limits are not unique regardless of the path we take to approach \((0,0)\), hence \(f'(0)\) does not exist. Therefore, an equation can satisfy the Cauchy-Riemann equations at \(0,0\), yet have a derivative that does not exist. The Cauchy-Riemann equations does not guarantee differentiability at \(z_0 = 0\).
		}
