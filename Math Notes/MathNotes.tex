...
%				enlargelimits=0.3,
	...
	\begin{theorem}[Cauchy-Riemann Equations (Polar)]
		...
		Let \(f(z) = u(r, \theta) + iv(r,\theta)\) be defined in some neighbourhood \(\epsilon\) of \(z_0 = r_0 e^{i\theta_0}\), $z_0 \neq 0$. If the first order partials derivatives of \(u\) and \(v\) with respect to \(r\) and \(\theta\) exists and satisfies the polar form of the Cauchy-Riemann equations: 
		\begin{align*}
		...
		\begin{align*}
			f'(z_0) = \eval{e^{i\theta}(u_r + iv_r)}_{(r_0, \theta_0)}
					= \eval{\frac{e^{i\theta}}{r}(v_\theta - iu_\theta)}_{(r_0, \theta_0)}
		\end{align*}
	\end{theorem}
	\begin{proof}
		Let \(f(z) = u(r,\theta) + iv(r,\theta)\). Suppose that the first order partial derivatives of \(u\) and \(v\) exists in some neighbourhood \(\epsilon \) of \(z_0\) and is continuous at \(z_0\). By differentiating \(u\) and \(v\) with respect to \(x\) and \(y\):
		\begin{align*}
	...
	\begin{question}
		...
		\end{align*}
		Should these be $\iff$ instead of $\implies$? 
		No! As we will see in \cref{f(z) satisfy Cauchy-Riemann but f'(z_0) does not exist Example - Complex}.
	\end{question}
	...
	\label{f(z) satisfy Cauchy-Riemann but f'(z_0) does not exist Example - Complex}
		...
		\textcolor{Grey}{\textbf{Along the real axis:}}
		
	
		\textcolor{Grey}{\textbf{Along the imaginary axis:}}
		
		
		\textcolor{Grey}{\textbf{Along \(\Delta x = \Delta y\):}}
	...
	\begin{example}(Any branch of \(f(z) = z^{1/2}\) is differentiable everywhere in domain of definition)
		...
		\end{align*}
		Thus, the derivative exists wherever \(f(z)\) is defined. Also
		\begin{align*}
			f'(z) 
