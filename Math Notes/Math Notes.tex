...
%			\legend{$z = x + iy$}
	...
	Taking the $m$-th root of $z \in C$ scales $r$ and $\theta$ by $1/m$, and provides solutions at $m$ equal parts of the circle. 
	
	\begin{example}
		Consider $z = 32 e^{i(5/6)\pi}$, then $z^{(1/5)} = 3e^{i(\pi/6) + i(2/5) n \pi}$ for $n \in \mathbb{Z}$. Before and after graphs are as follows: 
