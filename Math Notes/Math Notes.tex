...
%			\legend{$z = x + iy$}
	...
	An important property of polynomials is observed when \cref{Triange Inequality - Complex} is applied to polynomial.
	
	Consider the polynomial $P(z)$ where $a_n \in \mathbb{C}$, $n \in \mathbb{N}$, and $a_0 \neq 0$.
	$$P(z) = a_0 + a_1 z + a_2 z^2 + \ldots + a_n z^n$$
	Then $\forall \abs{z} < R, R \in \mathbb{R}_{>0}$: 
