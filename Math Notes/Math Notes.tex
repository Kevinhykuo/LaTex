...
%			\legend{$z = x + iy$}
	...
	\begin{example}
		Consider $z = 32 e^{i(5/6)\pi}$, then $z^{(1/5)} = 3e^{i(\pi/6) + i(2/5) n \pi}$ for $n \in \mathbb{Z}$. The radius went from $35$ to $35^{(1/5)} = 2$, and five roots appear equally spaced with distance of $(2/5)\pi$ on a circle with radius $2$. Before and after graphs are as follows, note graphs are not to scale: 
		\begin{figure}[H]
			\centering
