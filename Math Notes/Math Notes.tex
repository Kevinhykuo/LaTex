...
%			\legend{$z = x + iy$}
	...
	\begin{definition}[Roots of $z$]
		\label{Roots of z Definition - Complex}
		Consider any $z \in \mathbb{C}$ and any $m \in \mathbb{N}$.
		$$z^{\frac{1}{m}} = r^\frac{1}{m} e^{i\left(\frac{\theta + 2n \pi}{m}\right)} = r^\frac{1}{m} e^{i \left(\frac{\theta}{m} + \frac{2n \pi}{m}\right)}$$
	\end{definition}  
	
	Taking the $m$-th root of $z \in C$ scales $r$ and $\theta$ by $1/m$, and provides solutions at equally spaced by $2\pi / m$ on the circle. 
	
	\begin{example}
		Consider $z = 32 e^{i(5/6)\pi}$, then $z^{(1/5)} = 3e^{i(\pi/6) + i(2/5) n \pi}$ for $n \in \mathbb{Z}$. The radius went from $35$ to $35^{(1/5)} = 2$, and five roots appear equally spaced with distance of $(2/5)\pi$ on a circle with radius $2$. Before and after graphs are as follows, note graph on right is zoomed in: 
		\begin{figure}[H]
			\centering
...
%					ytick distance=2,
					...
					coordinates {
						(1.73205, 1) [$z = 2e^{i\pi/6}$]
						(-0.4158, 1.9563) [$z = 2e^{i17\pi/30}$]
						(-1.9890, 0.20906) [$z = 2e^{i29\pi/30}$]
						(-0.8135, -1.8271) [$z = 2e^{-i19\pi/30}$]
						(1.4863, -1.3383) [$z = 2e^{-i7\pi/30}$]
					};
